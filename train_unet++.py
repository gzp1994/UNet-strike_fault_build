...
# checkpoint
filepath="check/unetpp.{epoch:02d}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc',
        verbose=1, save_best_only=False, mode='max')
logging = TrainValTensorBoard()
reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.2, patience=20, min_lr=1e-8)
callbacks_list = [checkpoint, logging, reduce_lr]

sxpath = './train/data1/'
kxpath = './train/label1/'
n1,n2,n3=128,128,128
tdata_ids = range(0,200)
vdata_ids = range(200,220)
...
# Fit the model
history = model.fit_generator(generator=train,validation_data=valid,epochs=100, callbacks=callbacks_list, verbose=1)
print(history)
